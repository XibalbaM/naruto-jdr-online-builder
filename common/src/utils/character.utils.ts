...
export function baseLevel(character: _Character, baseShortName: string, bases: Base[]): number {
    return bases.find(base => base.shortName === baseShortName)?._id || 0;
}

export function chakraControl(character: _Character, bases: Base[]): number {
    return baseLevel(character, 'COR', bases) + baseLevel(character, 'ESP', bases);
}

export function chakraRegen(character: _Character, clans: _Clan[], bases: Base[], chakraSpes: _ChakraSpe[]): number {
    return Math.floor(maxChakra(character, clans, bases, chakraSpes) * (1 + chakraSpeAmount(character, 'Inépuisable', chakraSpes)) / 100);
}
...
export function maxChakra(character: _Character, clans: _Clan[], bases: Base[], spes: _ChakraSpe[]): number {
    let clanBonus = 0;
    ...
    return (50 * chakraControl(character, bases))
...
export function maxChakraSpes(character: _Character, bases: Base[]): number {
    const chakraControlValue = chakraControl(character, bases);
    let maxChakraSpes = 1;

export default [
    {
        "title": "ACCROISSEMENT DU CHAKRA (1)",
        "cost": "1 point de Nindo",
        "time": "Momentané, 5 tours",
        "description": "Accroître temporairement son potentiel de chakra de 500 points pendant 5 tours.",
    },
    {
        "title": "DÉPASSEMENT (1)",
        "cost": "1 point de Nindo",
        "time": "5 charges",
        "description": "Gagner cinq charges pouvant être dépensées au choix en:\n" +
            "**1. Bonus Critique**" +
            "Une charge = un bonus de +5 sur un jet, quel qu’il soit. Deux charges max sur un même jet, annoncer les bonus avant le jet.\n" +
            "**2. Déluge de Kunai**" +
            "Une charge = une interception.\n" +
            "**3. Relance**" +
            "Une charge = relancer un jet de dé et garder le meilleur des deux scores."
    },
    {
        "title": "ÉVEIL (5)",
        "cost": "5 points de Nindo",
        "time": "Instantané",
        "description": "Gagner trois actions retardées et/ou de lignée, à jouer obligatoirement dans le tour où l’Éveil est activé. Le personnage accède à l’ensemble des pouvoirs de sa lignée."
    },
    {
        "title": "LIER NOS DESTINÉES (1, 2, 3)",
        "cost": "1 point de Nindo pour chaque palier, pour un maximum de 3 pour chaque Lien.",
        "time": "Permanent",
        "description": "**1. Notre lien**" +
            "1/session, le Lien peut offrir son aide, donner des conseils ou intervenir positivement dans le cadre de ses capacités.\n" +
            "**2. Notre histoire**" +
            "1/session, le Lien peut faire avancer d’un segment un arc narratif par le biais d’une petite scène (dialogue, flashback, etc), en plus de “Notre lien”.\n" +
            "**3. Notre destin**" +
            "En addition de “Notre lien” et “Notre histoire”, le Lien ouvre un nouvel arc narratif qui lui est propre et s’ajoute aux arcs narratifs du personnage."
    },
    {
        "title": "MON CORPS EST UN BOUCLIER (1)",
        "cost": "1 point de Nindo",
        "time": "Instantané",
        "description": "1. Annuler ou hériter d’une Condition, subir deux paliers de Blessures.\n" +
            "2. Annuler ou hériter d’un palier de Blessure, subir deux Conditions."
    },
    {
        "title": "UNISON (1)",
        "cost": "1 point de Nindo",
        "time": "Instantané",
        "description": "Gagner une Opportunité, à jouer immédiatement."
    }
];
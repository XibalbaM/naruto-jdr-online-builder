...
export default async function (model: Model<any>, modelToClass: (model: any) => any) {
    ...
    router.put("/:id", authMiddleware(), adminMiddleware(), contentMiddleware({data: {}}), dataController.update);
    router.delete("/:id", authMiddleware(), adminMiddleware(), dataController.delete);
